...
class NmapScanner(object):
    ...
    def check_ports(self, ip, ports):
        try:
            ...
            logging.warning("Process stopped as TERMINATE Signal received")
    ...
    def is_online(self, ip):
        try:
            ...
            # FIX [BUG-06]: was silently passing without returning False
            return False
        else:
            if result == 'up':
                return True
    ...
    def _run(self, ip_str, arg_str):
        if arg_str in ("-sS", "-sN", "-sF", "-sX", "-O", "-f"):
            euid = os.geteuid()
            if euid != 0:
                logging.warning("Requires root permission")
                return None
            else:
                pass
